...
-- OPTIMIZATION: Only run rainbow loop when UI is visible
local function IsAnyWindowVisible()
    for _, window in pairs(Library.OpenedFrames) do
...
local function StartRainbowLoop()
    ...
    rainbowConn = RenderStepped:Connect(function()
        if not IsAnyWindowVisible() then return end
        local Tick = tick();
        local Hue = Library.CurrentRainbowHue or 0
        Hue = Hue + (1 / 400)
...
-- OPTIMIZATION: Use a single global InputService.InputBegan connection for all toggle keybinds
local ToggleKeybinds = {}
InputService.InputBegan:Connect(function(input, processed)
    if processed then return end
    for _, toggle in pairs(ToggleKeybinds) do
        if toggle.Keybind and input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode.Name == toggle.Keybind then
            toggle.Value = not toggle.Value
            toggle:Display()
            if toggle.Changed then toggle.Changed() end
            Library:AttemptSave()
        end
    end
end)
...
-- OPTIMIZATION: Only update colors for visible elements
function Library:UpdateColorsUsingRegistry()
    for Idx, Object in next, Library.Registry do
        if Object.Instance.Visible ~= false then
...
do
    ...
    function Funcs:AddButton(Text, Func)
        ...
        function Button:Remove()
            self.ButtonOuter:Destroy()
        end

        Button.ButtonLabel = ButtonLabel;
        Button.ButtonOuter = ButtonOuter;
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        table.insert(Toggle._connections, KeybindButton.MouseButton1Click:Connect(function()
            KeybindButton.Text = '[...]'
            waitingForKey = true
        end))
        table.insert(Toggle._connections, InputService.InputBegan:Connect(function(input, processed)
            if waitingForKey and not processed then
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    Toggle.Keybind = input.KeyCode.Name
                    KeybindButton.Text = '['..Toggle.Keybind..']'
                    waitingForKey = false
                end
            end
        end))
        ToggleKeybinds[Toggle] = Toggle
...
function Library:CreateWindow(WindowTitle)
    ...
    function Window:AddTab(Name)
        ...
        -- Replace LeftSide and RightSide with ScrollingFrames for scrollability
        local LeftSide = Library:Create('ScrollingFrame', {
            ...
            CanvasSize = UDim2.new(0, 0, 1, 507);
        ...
        -- Helper to update CanvasSize for scrolling
        local function UpdateScrollCanvas(Side)
            local totalY = 0
            for _, child in ipairs(Side:GetChildren()) do
                if child:IsA('Frame') and child.Visible and child.Name ~= '' then
                    totalY = totalY + child.Size.Y.Offset + 8 -- 8 is padding
                end
            end
            Side.CanvasSize = UDim2.new(0, 0, 0, math.max(totalY, 507))
        end

        -- Patch AddGroupbox to update CanvasSize after adding
        local oldAddGroupbox = Tab.AddGroupbox
        function Tab:AddGroupbox(Info)
            local Groupbox = oldAddGroupbox(self, Info)
            task.defer(function()
                if Info.Side == 1 then
                    UpdateScrollCanvas(LeftSide)
                else
                    UpdateScrollCanvas(RightSide)
                end
            end)
            return Groupbox
        end
